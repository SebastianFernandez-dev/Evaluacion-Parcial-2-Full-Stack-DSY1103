package com.dsy1103.mspagos.exception;
...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ErrorResponse> handleFeignException(FeignException ex, HttpServletRequest request) {
        ...
        ErrorResponse error = ErrorResponse.builder()
                .timestamp(LocalDateTime.now())
                .status(status.value())
                .error("Error al comunicarse con ms-pedidos")
                .message("El PEDIDO con el ID indicado no existe o el servicio no está disponible")
                .path(request.getRequestURI())
                .build();
        log.error("Feign error en {}: {}", request.getRequestURI(), ex.getMessage());
